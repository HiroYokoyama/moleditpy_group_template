...
A static palette of ~300 substituent abbreviations (Me, Ph, Boc, Ts, TBS, ...).
...
existing atom, the group is attached to it.
...
from .palette_dialog import GroupPaletteDialog
...
def show_palette(context):
    ...
    window = context.get_window(_WINDOW_ID)
    if window:
        window.show()
        window.raise_()
    ...
    context.register_window(_WINDOW_ID, dialog)
    dialog.show()
    return dialog
